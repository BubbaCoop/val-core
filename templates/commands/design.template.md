...
3. Write manifest.json:
   ...
   Record the model you are running under as `"model"` — every stage inherits it.
   Record it **again on every gate record** as `"model"`, read at the time that gate
   returns rather than copied from the top. A single top-level field states an intention;
   a per-gate field is the only thing that proves no stage silently ran on another model,
   which has already happened once. A gate whose model differs from the run's is a
   finding, not a detail: say so in the writeup.
...
5. **Confirm you are running in the right repo, with this repo's agents.** Two checks,
   both cheap, both fatal:
   - The run directory you are about to create must be inside the session's working
     directory. `pwd` and compare. Driving a run in repo A using agents generated for repo
     B produces output that looks right and was composed against the wrong library: the
     agents carry another library's class map, methodology path and package name.
   - The generated agents must belong to THIS library. Read the
     `GENERATED by @valiify/val-core@<v> from val/config.json (library: <name>)` header of
     `.claude/agents/design-concept-architect.md` and compare `<name>` against
     `library.name` in `val/config.json`. If they differ, the session is running another
     repo's agents — STOP and tell the requester to run `npx val-init` here and start a new
     session.
   Record both in the manifest's gate-0 note, with the values you compared.
6. **Refuse to run in plan mode.** Every gate of this pipeline writes files —
...
`{ "gate": 1, "agent": "design-brief-intake", "status": "pass", "at": ISO, "model": "<the model that stage ran on>", "notes" }`.
...
2. Ask IN CHAT, all questions in one message, grouped by TRIGGER — and relay each one
   **verbatim, in its six-line shape**:
   ...
   Reproduce every line, in that order, with the subagent's own wording. Do NOT
   paraphrase a question into prose, do NOT summarise several into one, and do NOT
   decorate the message with progress framing — no percentage complete, no checkmark
   inventory of what already passed, no "just one small thing". Those read as a status
   report, which invites a yes/no; the shape exists because each line is a separate thing
   the requester has to see, and because a machine reads it. You may add one sentence
   before the block saying which gate is paused and one after saying what resumes on an
   answer. Nothing else.

   A `§9-forbidden` question is a choice for the requester: drop the ask, or record a
   deliberate methodology exception that the writeup will carry — never a silent
   compromise.
